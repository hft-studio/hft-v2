...
export async function buyAsset(params: {
    ...
    const route = await generateRoute({
        ...
        rawAmount: params.usdcAmount.toString()
    });
    if (!route) {
    ...
    const amount = BigInt(receipt.logs[1].data);
    return amount;
}
...
export async function depositInGauge(params: {
    ...
    smartAccount: EvmSmartAccount;
    lpTokenAddress?: string; // Add this new optional parameter
}) {
    const poolData = await getPoolData(params.poolId);
